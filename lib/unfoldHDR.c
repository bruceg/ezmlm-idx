	...
	/*         3 = reply & pefix      */
{
  ...
  cpend = cp + n - 1;
  cpnext = cp;
  stralloc_copys(&tmpdata,"");
  stralloc_ready(&tmpdata,n);
  ...
  if(!case_diffb(charset,11,"iso-2022-jp")) {
    ...
    while (cp < cpend) {
      ...
      if (*cp == ESC)			/* maybe another G0 designation */
        if (*(cp+1) == '(' || *(cp+1) == '$') {	 /* yep! */
          stralloc_catb(&tmpdata,cpnext,cpesc-cpnext);
          cpnext = cp;
	  continue;
      }
      if (reg == newreg) {
    ...
    stralloc_catb(&tmpdata,cpnext,cpend - cpnext + 1);
    if (reg != 6) {	/* need to return to us-ascii at the end of the line */
      stralloc_cats(&tmpdata,TOASCII);
    ...
    while (++cp <= cpend) {
      if (*cp == SI || *cp == SO) {
        if (state == *cp) {		 /* already in state. Skip shift seq */
          stralloc_catb(&tmpdata,cpnext,cp-cpnext-1);
          cpnext = cp;
        } else				/* set new state */
          state = *cp;
        if (++cp > cpend) break;
